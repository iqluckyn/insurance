...
import { getEntities } from './farm.reducer';
...
export const Farm = () => {
  ...
  const totalItems = useAppSelector(state => state.farm.totalItems);
    ...
    <div>
      ...
      <div className="table-responsive">
        {farmList && farmList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('cellIdentifier')}>
                  Cell Identifier <FontAwesomeIcon icon={getSortIconByFieldName('cellIdentifier')} />
                </th>
                <th>
                  Crop Type <FontAwesomeIcon icon="sort" />
                </th>
                <th>
            ...
            <tbody>
              {farmList.map((farm, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{farm.cellIdentifier}</td>
                  <td>{farm.cropType ? <Link to={`/crop-type/${farm.cropType.id}`}>{farm.cropType.cropName}</Link> : ''}</td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/farm/${farm.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/farm/${farm.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">No Farms found</div>
        )}
      </div>
      {totalItems ? (
        <div className={farmList && farmList.length > 0 ? '' : 'd-none'}>
